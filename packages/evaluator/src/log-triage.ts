...
/** One optional sample rubric. Application work should pass its own `questions`. */
...
export function skipProtectedLog(input: JsonValue): EvaluatorSkipResult | undefined {
...
export function skipOversizedLog(input: JsonValue): EvaluatorSkipResult | undefined {
  if (!isJsonObject(input)) {
    return undefined
  }
...
export function skipLogTriageModel(input: JsonValue): EvaluatorSkipResult | undefined {
  return skipProtectedLog(input) ?? skipOversizedLog(input)
}

export function routeLogTriage(
...
/** Optional sample rubric. Use your own `questions` for application work. */
export function defineLogTriage(options: { name?: string; retainBelow?: number } = {}) {
  ...
  return defineEvaluator({
    ...
    description: 'Example rubric: score a record before a more expensive step.',
