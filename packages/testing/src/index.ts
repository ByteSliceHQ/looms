...
function namedEffects(effects: EffectBag): NamedEffect[] {
  ...
  for (const key of Object.keys(effects)) {
    ...
    if (!value || !Predicate.isString(value.type)) continue
    listed.push({
      type: value.type,
...
export function moduleConformance(module: AnyRuntimeModule): string[] {
  const errors: string[] = []
  if (!module.namespace) errors.push('namespace required')
  if (!module.protocolVersion) errors.push('protocolVersion required')
