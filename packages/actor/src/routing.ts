...
import { createRunId, isJsonObject, stringifyJson, type JsonValue } from '@looms/core'
...
export class RunTargetError extends Data.TaggedError('RunTargetError')<{
  ...
  constructor(method: string, path: string) {
    super({
      method,
      path,
      message: `Cannot resolve runId from request: ${method} ${path}`,
    })
 ...
 */
export function resolveRunTarget(req: Request): Promise<ResolvedRunTarget> {
  return Effect.runPromise(
    Effect.gen(function* () {
      ...
      if (req.method === 'POST' && url.pathname === '/runs') {
        const text = yield* Effect.promise(() => req.clone().text())
        ...
        const request = new Request(req.url, {
          method: req.method,
          headers,
          body: stringifyJson(payloadObj),
        })

        return { runId, request }
      }

      return yield* new RunTargetError(req.method, url.pathname)
    }),
  )
}
