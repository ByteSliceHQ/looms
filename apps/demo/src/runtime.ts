...
export function demoModules(options: { llm?: LlmAdapter; agents?: readonly AgentDefinition[] } = {}) {
  return [agent({ llm: options.llm ?? demoLlm, definitions: [...definitions.filter(d => d.kind === 'agent'), ...(options.agents ?? [])] }), workflow({ definitions: definitions.filter(d => d.kind === 'workflow') }), approval(), payments] as const
}
